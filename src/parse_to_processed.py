...
# parse the corpus
corpus = parse_project.parse_project(
    Path(settings.DATA_RAW_DIRP) / settings.DATA_XMI_EXPORT_DIRN
)
...
for k, v in corpus.items():
    for doc in v:
        for i, sentence in enumerate(doc.sentences):
            ...
            subtype_labels = [
                f"{ev.event_type}.{ev.event_subtype}" for ev in sentence.events
            ]
            instance = {
...
dataset_df.to_csv(
    Path(settings.DATA_INTERIM_DIRP) / "dataset.tsv", sep="\t", index=False
)
...
dataset_df.to_csv(
    Path(settings.DATA_PROCESSED_DIRP) / "dataset_event_type.tsv", sep="\t", index=False
)
with open(
    Path(settings.DATA_PROCESSED_DIRP) / "type_classes_multilabelbinarizer.json", "wt"
) as classes_out:
...
dataset_df.to_csv(
    Path(settings.DATA_PROCESSED_DIRP) / "dataset_event_subtype.tsv",
    sep="\t",
    index=False,
)
with open(
    Path(settings.DATA_PROCESSED_DIRP) / "subtype_classes_multilabelbinarizer.json",
    "wt",
) as classes_out:
    json.dump(mlb.classes_.tolist(), classes_out)
