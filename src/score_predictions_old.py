...
model_dirn = "2019-12-26_22-39-44-roberta-large"
# model_dirn = "2019-12-18_16-59-12-roberta-large" # 16 epochs
model_dirp = model_dir / model_dirn

pred_fp = model_dirp / "holdouttest_predictions.tsv"
...
testset_df = pd.read_csv(
    pred_fp, sep="\t", converters={"labels": literal_eval, "y_pred": literal_eval}
)

testset_df["y_pred_norm"] = np.array(
...
testset_df["labels_pred"] = mlb.inverse_transform(
    np.array(testset_df["y_pred_norm"].to_list())
)
...
for conf, label in zip(cm, classes):
    record = (
        label,
        {"tn": conf[0, 0], "tp": conf[1, 1], "fn": conf[1, 0], "fp": conf[0, 1],},
    )
    pprint(record)
...
train_labels = np.array(
    dataset_df[dataset_df["dataset"] == "silver"]["labels"].to_list()
)
