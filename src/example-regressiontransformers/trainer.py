...
class TransformerRegressionTrainer:
    ...
    def __init__(
        ...
        self.datasets, self.dataloaders = self._set_data_loaders(
            train_files, valid_files, test_files, batch_size
        )
    ...
    def _set_data_loaders(train_files, valid_files, test_files, batch_size):
        ...
        dataloaders = {
            "train": DataLoader(
                datasets["train"], batch_size=batch_size[0], shuffle=False
            )
            if train_files is not None
            else None,
            "valid": DataLoader(
                datasets["valid"], batch_size=batch_size[1], shuffle=False
            )
    ...
    def _set_device(gpu_ids):
        ...
        if main_device.type == "cuda":
            if (
                1 < len(gpu_ids) <= torch.cuda.device_count()
    ...
    def train(self, epochs=10, checkpoint_f="checkpoint.pth", log_update_freq=10):
        ...
        for epoch in range(epochs):
            ...
            try:
                valid_pearson = pearsonr(
                    valid_results["predictions"], valid_results["labels"]
                )
            except FloatingPointError:
            ...
            if valid_loss <= valid_loss_min:
                logging.info(
                    f"!! Validation loss decreased ({valid_loss_min:.6f} --> {valid_loss:.6f})."
                )
                ...
                if train_loss <= valid_loss:
                    logging.warning(
                        f"!! Training loss is lte validation loss. Might be overfitting!"
                    )
            ...
            if self.patience:
                if (epoch - last_saved_epoch) == self.patience:
                    logging.info(
                        f"Stopping early at epoch {epoch} (patience={self.patience})..."
                    )
                    break
        ...
        logging.info(
            f"Training completed in {(time.time() - train_start):.0f} seconds"
    ...
    def _process(self, do, log_update_freq, epoch=None):
        ...
        if log_update_freq:
            ...
            update_checkpoints = {
                int(nro_batches - (i * update_interval))
                for i in range((100 // log_update_freq))
            }
        ...
        for batch_idx, data in enumerate(self.dataloaders[do], 1):
            ...
            if log_update_freq and batch_idx in update_checkpoints:
                if do in ("train", "valid"):
                    logging.info(
                        f"{do.capitalize()} epoch {epoch}, batch nr. {batch_idx}/{nro_batches}..."
                    )
                else:
                    logging.info(
                        f"{do.capitalize()}, batch nr. {batch_idx}/{nro_batches}..."
                    )
    ...
    def test(self, checkpoint_f=None, log_update_freq=0):
        ...
        logging.info(
            f"Testing completed in {(time.time() - test_start):.0f} seconds"
