...
def make_score_summaries(model_dirp, classes):
    # Load runs + paths
    run_pred_fps = list(model_dirp.rglob("fold*/testset_with_predictions.tsv")) + list(
        model_dirp.rglob("holdout/testset_with_predictions.tsv")
    )
    ...
    for testset_fp in run_pred_fps:
        ...
        testset_df.to_csv(
            testset_proc_fp, sep="\t", index=False
    ...
    # std + mean in one df
    fold_bin_df.loc["mean"] = fold_bin_avg_df
    fold_bin_df.loc["std"] = fold_bin_df.std(axis=0)
    fold_proba_df.loc["mean"] = fold_bin_avg_df
    fold_proba_df.loc["std"] = fold_proba_df.std(axis=0)

    # write all fold score
    fold_bin_df.to_csv(model_dirp / "bin_scores_by_fold_mean_std.tsv", sep="\t")
    fold_proba_df.to_csv(model_dirp / "proba_scores_by_fold_mean_std.tsv", sep="\t")
    ...
    summary_df = summary_df.join(fold_bin_df.loc["std"].rename("Crossval_bin_std"))
    summary_df = summary_df.join(fold_proba_df.loc["std"].rename("Crossval_proba_std"))
    ...
    print(summary_df)
    print(
        f"{model_dirn.upper()} holdout and crossvalidation scores by class label summary:"
    )
    print(summary_types_df)

    return summary_df, summary_types_df


if __name__ == "__main__":

    # 1. Set model path and load labels and predictions
    model_dir = Path(settings.MODEL_DIRP)
    ...
    # model_dirn = "2020-01-10_14-53-30-roberta-large" # 10fold 8 epoch WORKING
    # model_dirn = "2020-01-12_14-47-10-xlnet-base-cased" # 10fold
    # model_dirn = "2020-01-12_14-36-38-roberta-large" # 10 fold 12 epoch RUN STOPPED
    # model_dirn = "2020-01-13_15-24-19-xlnet-base-cased" # XLNET 10fold 32 epoch PRETTY GUD
    # model_dirn = "xlnet-large-cased_epochs-8" # 10 fold run xlnet 8 epoch
    # model_dirn = "xlnet-large-cased_epochs-16" # 10 fold run xlnet 16 epoch
    # model_dirn = "xlnet-large-cased_epochs-32" # 10 fold run xlnet GOOD SCORE IN CROSSVAL BUT OVERFIT WHEN TESTING HOLDOUT
    # model_dirn = "2020-01-14_15-26-20-roberta-large" # test run
    model_dirn = "roberta-large_epochs-16"  # 10 fold run 16 epoch PRETTY GOOD RESULTS

    model_dirpath = model_dir / model_dirn

    # 2. load classes json as made by parse_to_processed.py
    with open(
    ...
    summary_df, summary_types_df = make_score_summaries(model_dirpath, classes)
