...
class TransformerRegressionPredictor:
    ...
    def predict(self):
        ...
        """
        for drop, lr in product(self.config['model']['dropout'],
            ...
            trainer = TransformerRegressionTrainer(model=model,
                                                   tokenizer=tokenizer,
                                                   optimizer=optimizer,
                                                   scheduler=scheduler,
                                                   **opts['training']['files'],
    ...
    def get_optim(optim_obj, model):
        ...
        if optim_name.lower() == 'adamw':
            return AdamW([p for p in model.parameters() if p.requires_grad], **optim_copy), optim_name
        elif optim_name.lower() == 'adam':
            return optim.Adam([p for p in model.parameters() if p.requires_grad], **optim_copy), optim_name
        else:
            raise NotImplementedError('This optimiser has not been implemented in this script.')
